...
#
# These started out embedded in the instance's user data, which EC2 caps at
# 16 KB. The three configs plus the bootstrap script came to 16.7 KB with a
...
# stamped into the user data, so changing a config still replaces the instance:
# it never keeps running a configuration it was not built from.
